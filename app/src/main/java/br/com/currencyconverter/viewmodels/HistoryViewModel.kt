...
constructor(private val repository: CurrencyRepository)
    : ViewModel()
{
    private var _data = MutableLiveData<List<History>>()
    ...
    private val observer = Observer<ViewState<List<History>>>{ res->
        when(res){
            is ViewState.Error -> {
    ...
    fun load(currencyA: String,currencyB: String,amount:String){
        amount.toFloatOrNull()?.let {
            _loading.postValue(true)

            val source =  repository.history(currencyA,currencyB,it)
            source.removeObserver(observer)
            source.observeForever(observer)
        } ?: kotlin.run {
            _message.postValue("Value is wrong")
        }
